...
// Only a backend on this same host can be attributed to this host's GPU. A
// Xavier running Signal K is perfectly entitled to ask an Orin across the boat
// network for TensorRT-LLM answers, and warning about the local GPU then would
// be simply wrong.
