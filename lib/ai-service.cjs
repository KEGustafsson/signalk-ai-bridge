...
// Measured against gemma4:e2b-it-qat on this vessel's context, not guessed:
// - The alarm rule exists because a 4B model fumbles negation. Asked whether
//   any alarm was abnormal, it answered "No alarms are in a normal state" -
//   the opposite of the truth - in two of four sampler configurations. Giving
//   it the exact affirmative sentence to use removed the inversion in every
//   later run.
// - The brevity rule exists because "Is everything okay?" once returned a
//   794-token recital of the entire context. Useful data was in there, but an
//   operator asking a yes/no question at the helm needs the yes/no first.
// - No rounding instruction, deliberately. A trial "round to one decimal"
//   rule made the model degrade 13.43 V to "13.4" with the unit dropped;
//   values are already rounded server-side (roundForPrompt), so the model is
//   told to repeat them as given.
const DEFAULT_SYSTEM_PROMPT =
  'You are Signal K AI Bridge, a maritime assistant for vessel operators. ' +
  'Use only the provided Signal K context, and report values as given, with their units. ' +
  'Be explicit when context is missing or stale. ' +
  'About alarms, avoid double negatives: if every notification state is "normal", say "All alarms are normal"; ' +
  'otherwise list only the abnormal ones with their state and message. ' +
  'Answer only what was asked, briefly, in plain prose and short bullets; no arithmetic working, no LaTeX. ' +
...
// Halvings allowed after an out-of-memory failure. Three takes a 8192/2048
// working set down to 1024/256 - below the backend's own defaults - which is
// far enough that a fourth attempt would only add another model reload to a
// question that is not going to fit.
...
// Both caps above are absolute, and that was the whole problem: they were sized
// for the default num_ctx 8192 and applied unchanged at any other window. At
// num_ctx 4096 the plugin still built a 12,000-character context, llama.cpp
// truncated the prompt to what num_predict left it, and the "N further paths
// omitted" note - the plugin's own honesty about dropped data - was absent
// precisely when data was being dropped. The budget now follows the window.
//
// 2.5 characters per token is measured on this prompt shape, not a general
// rule: compact Signal K JSON is mostly punctuation, digits and dotted path
// names, all of which tokenize far denser than prose. A vessel snapshot of
// 12,859 characters came back from Ollama as 4,949 prompt tokens (2.6), and
// rounding down keeps the estimate on the safe side of the real tokenizer.
const CONTEXT_CHARS_PER_TOKEN = 2.5;

// The system prompt, the operator's question and the response-requirements
// block, none of which the context budget may claim.
const PROMPT_OVERHEAD_TOKENS = 400;

// Below this there is no useful vessel context left, only the note saying so.
const MIN_CONTEXT_CHARS = 500;

/**
 * Characters of Signal K context that fit in this configuration's window.
 *
 * `undefined` config (the exported `buildAiMessages` is called from tests and
 * from the panel's request preview) keeps the historical absolute cap.
 */
function contextCharBudget(config) {
  if (typeof config !== 'object' || config === null || typeof config.numCtx !== 'number') {
    return MAX_CONTEXT_CHARS;
  }

  const promptTokens = promptTokenBudget(config) - PROMPT_OVERHEAD_TOKENS;
  return Math.max(
    MIN_CONTEXT_CHARS,
    Math.min(MAX_CONTEXT_CHARS, Math.floor(promptTokens * CONTEXT_CHARS_PER_TOKEN))
  );
}

/**
 * The history header, trimmed to fit its own budget.
 *
 * Only the series were ever counted, so the metadata around them could spend
 * the budget on its own: `requestedPaths` and `unavailablePaths` grow with the
 * operator's selection, and the window they are measured against is no longer
 * fixed - a small num_ctx can leave a history budget smaller than the two
 * lists together. That is how a prompt with no room for a single data point
 * still spent what room it had naming the paths that did not fit.
 *
 * `requestedPaths` goes first because it is the more inferable of the two: the
 * series that came back name themselves, while an absence is the part the
 * model cannot reconstruct from what it was given.
 */
const HISTORY_HEADER_LIST_KEYS = ['requestedPaths', 'unavailablePaths'];

function fitHistoryHeader(header, budget) {
  let fitted = header;

  for (const key of HISTORY_HEADER_LIST_KEYS) {
    if (JSON.stringify(fitted).length <= budget) {
      break;
    }
    const list = fitted[key];
    if (!Array.isArray(list) || list.length === 0) {
      continue;
    }
    fitted = {
      ...fitted,
      [key]: `${list.length} path(s), not listed here to fit the context window`
    };
  }
 ...
 */
function buildPromptHistory(history, budget = MAX_HISTORY_CONTEXT_CHARS) {
  if (typeof history !== 'object' || history === null) {
    return undefined;
  }

  const { series, ...rest } = history;
  const header = fitHistoryHeader(roundForPrompt(rest), budget);
  ...
  for (const [path, summary] of entries) {
    ...
    if (samples !== undefined && used + compact <= budget) {
      used += compact;
      kept[path] = stats;
      trimmedSeries += 1;
      continue;
...
function buildPromptContext(context, budget = MAX_CONTEXT_CHARS) {
  ...
  // History and the live snapshot share the window, so history takes at most
  // half of it - never more than the absolute cap - and the snapshot keeps the
  // rest. "What is happening now" must not be squeezed out by a long window.
  const historyBudget = Math.min(MAX_HISTORY_CONTEXT_CHARS, Math.floor(budget / 2));
  const history = buildPromptHistory(pruned.history, historyBudget);
  const dataBudget = Math.max(MIN_CONTEXT_CHARS, budget - (history ? JSON.stringify(history).length : 0));

  const entries = Object.entries(selectedData);
  let data = selectedData;
  let droppedPaths = 0;

  if (JSON.stringify(selectedData).length > dataBudget) {
    data = {};
    let used = 0;
    for (const [path, value] of entries) {
      const cost = path.length + JSON.stringify(value ?? null).length + 4;
      if (used + cost > dataBudget) {
        droppedPaths += 1;
        continue;
      }
      used += cost;
  ...
  // Alarms are the one absence the model will not leave alone. Asked "any
  // alarms?" with no notification path selected - and again with all 64
  // notifications.environment.* leaves dropped for want of budget - it
  // answered "all alarms are normal" both times, about data it had never been
  // shown. `unavailablePaths` cannot cover this: it lists configured paths
  // that came back empty, and a path the operator never configured is not in
  // it. Nothing else in the prompt says the snapshot has no alarm coverage,
  // and a fabricated all-clear is the one answer a vessel assistant must never
  // give.
  const hasNotifications = Object.keys(data).some(
    (path) => path === 'notifications' || path.startsWith('notifications.')
  );
  ...
  return {
    ...
    ...(hasNotifications
      ...
      : {
          alarms:
            'No notification data is included in this snapshot. Say that alarm status is unknown; ' +
            'do not state that alarms are normal or that there are none.'
        }),
...
function buildAiMessages(prompt, context, config) {
  ...
  const promptContext = buildPromptContext(context, contextCharBudget(config));
    ...
    },
    {
      ...
      content:
        ...
        (promptContext.history
          ...
          : '') +
        // Said unconditionally, because the failure it prevents is silent: the
        // values are already in operator units, and a model left to guess
        // converts them a second time. Measured with an operator system prompt
        // that asked for radians-to-degrees: an attitude yaw of 86.5 degrees
        // was reported as 495.
        'Units: values are already converted for the operator - angles in degrees, ' +
        'temperatures in degrees Celsius, speeds in knots. Everything else is in ' +
        'Signal K SI units (pressure in pascals, distance in metres, ratios 0-1). ' +
        'Do not convert any value again. A key ending in "@" is the timestamp of the ' +
        'path before it.\n\n' +
        'Response requirements:\n' +
        // Itemized on purpose, and placed here beside the question rather than
        // in the system prompt: a "say when data is missing" rule at the top
        // of the conversation did not stop the model from silently skipping a
        // requested fuel-tank level that the context lacked (three runs out of
        // three), while walking the request item by item forces each absence
        // to be confronted where it occurs.
        '- Answer every item the operator asked for, in the order asked; write "not available" for any item the context does not contain.\n' +
        '- Answer only what was asked; keep it brief.\n' +
        '- Complete every section you begin; do not stop mid-list or mid-sentence.\n' +
        '- If the answer has multiple sections, end with a one-line summary; a short answer needs none.'
    }
  ];
}
...
async function runChat(ollama, model, prompt, payload, config, runtimeConfig = config, disableThinking = false) {
  return ollama.chat({
    ...
    // runtimeConfig, not config: the memory ladder halves num_ctx between
    // attempts, and a prompt built for the configured window would then be
    // truncated by llama.cpp instead of pruned by the plugin. Measured on the
    // retry after an out-of-memory failure: num_ctx 4096 with the same
    // 4,927-token prompt, silently cut to 2,051 - so even a retry that fit
    // would have answered from half the vessel.
    messages: buildAiMessages(prompt, payload && payload.context, runtimeConfig),
    options: buildRuntimeOptions(runtimeConfig)
  });
}

/**
 * An out-of-memory failure that survived the retry, said usefully.
 *
 * The backend reports "CUDA error: out of memory" and stops there, which tells
 * an operator nothing about which of their settings to change. This names the
 * two that decide the allocation and the values the failed attempt used, so
 * the next step is a number to lower rather than a web search.
 */
function outOfMemoryAdviceError(cause, runtimeConfig) {
  const error = new Error(
    'The GPU ran out of memory even after retrying smaller ' +
      `(context window ${runtimeConfig.numCtx}, batch size ${runtimeConfig.numBatch}). ` +
      'Lower "GPU context window" (num_ctx) and "Prompt batch size" (num_batch) in the plugin settings, ' +
      'select fewer AI data paths, or free memory on the host.'
  );
  error.code = 'unknown';
  error.cause = cause;
  return error;
...
/**
 * The next smaller footprint to try, or undefined when both knobs are already
 * at their floor and there is nothing left to give up.
 */
function shrinkForRetry(runtimeConfig) {
  const numBatch = runtimeConfig.numBatch ?? DEFAULT_NUM_BATCH;
  const smaller = {
    ...runtimeConfig,
    numCtx: Math.max(MIN_NUM_CTX, Math.floor(runtimeConfig.numCtx / 2)),
    numBatch: Math.max(MIN_NUM_BATCH, Math.floor(numBatch / 2))
  };
  if (smaller.numCtx === runtimeConfig.numCtx && smaller.numBatch === numBatch) {
    return undefined;
  }
  return smaller;
}
...
/**
 * Retry an out-of-memory question at a progressively smaller footprint.
 *
 * One halving is not a ladder. A batch of 2048 against a context of 8192 is
 * sixteen times the default working set, so halving once still asks for eight
 * times what the backend's own defaults would, and the retry failed with the
 * identical CUDA error the first attempt did. Walking down until something
 * fits is what turns an unanswerable question into a slower answer.
 *
 * Deliberately not gated on `gpuAutoTune`: that switch governs the start-up
 * probe that decides the layer split, not whether a question that has already
 * failed for memory deserves a second, cheaper attempt. Gating it meant an
 * operator who pinned their own split got the raw CUDA error and no recovery.
 */
async function chatWithSmallerFootprint(
  ollama,
  model,
  prompt,
  payload,
  ...
  let attemptConfig = runtimeConfig;
  let lastError = initialError;

  for (let step = 0; step < MAX_MEMORY_RETRIES; step += 1) {
    const smaller = shrinkForRetry(attemptConfig);
    if (smaller === undefined) {
      break;
    }
    attemptConfig = smaller;
    try {
      return await runChat(ollama, model, prompt, payload, config, attemptConfig, disableThinking);
    } catch (retryError) {
      if (!isOutOfMemoryError(retryError)) {
        throw retryError;
      }
  ...
  // Out of memory at the smallest footprint we are willing to run: the raw
  // CUDA error names no setting, so say which ones decide it and what the
  // last attempt used.
  throw outOfMemoryAdviceError(lastError, attemptConfig);
}
...
async function queryOllamaModel(prompt, payload, config, dependencies) {
  ...
  try {
    ...
    if (isOutOfMemoryError(error)) {
      ...
      // or a longer context grew the cache. Retry smaller rather than failing
      // the operator's question outright.
      //
      // Both knobs, not just the context. num_ctx sizes the KV cache, but
      // num_batch sizes the prompt-eval compute buffers, and those scale with
      // the batch: at num_batch 2048 on an 8 GB Jetson they are the dominant
      // allocation, so halving the context alone freed nothing that mattered
      // and the retry failed exactly like the first attempt.
      response = await chatWithSmallerFootprint(
        ollama,
        modelUsed,
        prompt,
        payload,
      ...
      // The retry under the resolved tag is a first attempt of its own, so it
      // needs the same memory ladder the first one had. Without it an
      // out-of-memory failure here escaped raw - and it is the likely path on
      // a default install, whose untagged `gemma4` always misses once before
      // the real tag is found.
      try {
        response = await runChat(ollama, modelUsed, prompt, payload, config, runtimeConfig, disableThinking);
      } catch (resolvedError) {
        if (!isOutOfMemoryError(resolvedError)) {
          throw resolvedError;
        }
        response = await chatWithSmallerFootprint(
          ollama,
...
async function streamOllamaModel(prompt, payload, config, dependencies, onToken, signal) {
  ...
  const stream = await ollama.chat({
    ...
    keep_alive: config.keepAlive,
    ...(disableThinking ? { think: false } : {}),
    // Same reason as the blocking path: the options carry the tuner's window,
    // so the prompt has to be budgeted against that window and not the
    // configured one.
    messages: buildAiMessages(prompt, payload && payload.context, runtimeConfig),
    options: buildRuntimeOptions(runtimeConfig)
  });
...
/**
 * Whether the inference backend runs on this machine.
 *
 * Loopback is not the only way to say "here": a Signal K server told to reach
 * Ollama at the LAN address of its own board is still talking to its own GPU,
 * and reporting no board telemetry for it would be as wrong as reporting this
 * board's telemetry for someone else's. So the host is matched against every
...
function isLocalBaseUrl(baseUrl) {
  ...
  for (const addresses of Object.values(networkInterfaces())) {
    for (const address of addresses || []) {
      // A link-local IPv6 address carries a %zone suffix the URL does not.
      if (String(address.address).toLowerCase().split('%')[0] === host) {
        return true;
      }
    }
  }
...
async function getAccelerationReport(config, dependencies = {}) {
  // Board telemetry describes the machine this plugin runs on, which is only
  // the machine doing the inference when the backend is local. Read against a
  // remote Ollama it reported the wrong GPU's load, clock and temperature, and
  // told the operator to run nvpmodel and jetson_clocks on a board with no
  // model on it - advice that cannot help and hides the board that needs it.
  const boardIsTheBackend = isLocalBaseUrl(config.baseUrl);
  const [accelerator, jetson] = await Promise.all([
    ...
    boardIsTheBackend
      ...
      : Promise.resolve({
          present: false,
          message:
            `Inference runs on ${redactUrl(config.baseUrl)}, not on this machine. ` +
            'Board power mode, GPU clock and temperature are only reported when Signal K ' +
            'shares a host with the inference server.'
        })
  ]);
