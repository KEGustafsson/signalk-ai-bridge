...
async function queryOllamaModel(prompt, payload, config, dependencies) {
  ...
  try {
    ...
    if (isOutOfMemoryError(error) && config.gpuAutoTune) {
      ...
      // or a longer context grew the cache. Retry once smaller rather than
      // failing the operator's question outright.
      //
      ...
      runtimeConfig = {
        ...
        numBatch: Math.max(MIN_NUM_BATCH, Math.floor((runtimeConfig.numBatch ?? DEFAULT_NUM_BATCH) / 2))
      };
      try {
        response = await runChat(ollama, modelUsed, prompt, payload, config, runtimeConfig, disableThinking);
      } catch (retryError) {
        // Still out of memory at half of both: the raw CUDA error names no
        // setting, so say which ones decide it and what they were.
        if (isOutOfMemoryError(retryError)) {
          throw outOfMemoryAdviceError(retryError, runtimeConfig);
        }
        throw retryError;
      }
