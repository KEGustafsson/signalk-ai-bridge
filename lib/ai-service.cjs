 ...
 * a spinner.
 */
async function streamOllamaModel(prompt, payload, config, dependencies, onToken) {
  const ollama = getOllamaClient(config, dependencies);
  ...
  for await (const chunk of stream) {
    last = chunk;
    const fragment = extractTextContent(
      chunk && chunk.message && typeof chunk.message === 'object' ? chunk.message.content : undefined,
      { trim: false }
    );
...
async function streamAiModel(payload, config, dependencies = {}, onToken = () => {}) {
  ...
  try {
    return config.backend === 'tensorrt-llm'
      ...
      : await streamOllamaModel(prompt, payload, config, dependencies, trackedOnToken);
  } catch (error) {
    if (emitted || (error && (error.code === 'timeout' || error.code === 'validation-failed'))) {
      throw error;
    }
    if (isAbortError(error)) {
      throw createTimeoutError(config.requestTimeoutMs);
    }
    // Nothing was emitted, so the failure was at request time — a missing model
    // tag, say. The blocking path owns the resolution and retry ladder.
