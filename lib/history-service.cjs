...
function summarizeColumn(pathSpec, rows, columnIndex, samplesLimit) {
  ...
  if (numbers.length > 0) {
    ...
    summary.min = roundForPrompt(Math.min(...numbers));
    summary.max = roundForPrompt(Math.max(...numbers));
...
function summarizeHistoryPayload(payload, requestedSpecs, samplesLimit) {
  ...
  const columns = declared.length > 0
    ? declared.map((entry, index) => ({
        spec: requestedSpecs[index],
        path: typeof entry?.path === 'string' ? entry.path : parsePathSpec(requestedSpecs[index])?.path,
  ...
  columns.forEach((column, index) => {
    const key = column.path || column.spec;
    if (!key) {
      return;
    }

    const summary = summarizeColumn(column.spec || key, rows, index + 1, samplesLimit);
    if (!summary) {
  ...
  // A path the provider dropped from `values` entirely never got a column.
  for (const spec of requestedSpecs) {
    const parsed = parsePathSpec(spec);
    const key = parsed ? parsed.path : spec;
    if (!(key in series) && !unavailablePaths.includes(key)) {
      unavailablePaths.push(key);
    }
  }
