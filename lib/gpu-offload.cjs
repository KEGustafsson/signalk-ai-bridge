...
// at half the context.
//
// Deliberately broad. Both messages CAN mean something other than memory - a
// docker stop is also "signal: killed", a broken driver also fails cublasCreate
// - but the real messages carry no memory words at all, so demanding
// allocation evidence would make these patterns dead letters and un-fix the
// failures they were added for. Misclassifying costs one bounded retry at half
// the context, which then surfaces the original error unchanged; missing a
// real OOM costs the operator their answer.
