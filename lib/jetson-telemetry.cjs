...
/**
 * Compute capability per Tegra GPU node name.
 *
 * That node name is the only place on the board that identifies the GPU — there
 * is no nvidia-smi on L4T — and compute capability is what decides which
 * backends can run at all rather than merely run slowly. TensorRT-LLM compiles
 * for SM 8.0 and newer, so on a Xavier (7.2) or older it is not a configuration
 * to tune but an impossibility, and saying so beats an unexplained connection
 * failure.
 *
 * Deliberately excludes the bare `gpu`/`gpu.0` alias that some releases also
 * expose: it carries no generation, and guessing one from the board would be
 * worse than reporting nothing.
 */
const TEGRA_GPU_ARCHITECTURES = {
  ga10b: { architecture: 'Ampere', computeCapability: 8.7 }, // Orin
  gv11b: { architecture: 'Volta', computeCapability: 7.2 }, // Xavier
  gp10b: { architecture: 'Pascal', computeCapability: 6.2 }, // TX2
  gm20b: { architecture: 'Maxwell', computeCapability: 5.3 } // TX1 / Nano
};

const TEGRA_GPU_ARCH_NODE = /^(?:[0-9a-f]+\.)?(ga10b|gv11b|gp10b|gm20b)(?:\.\d+)?$/i;

// devfreq is included because it is where the node reliably appears under its
// generation name: /sys/devices also carries the generic `gpu.0` alias, and
// which of the two readdir returns first is not something to depend on.
const GPU_ARCH_DIRS = [...GPU_NODE_DIRS, 'sys/class/devfreq'];

/**
 * Identify a Tegra GPU generation from one device-tree node name.
 *
 * Accepts the decorations L4T puts on the node - an address prefix and an
 * instance suffix - so `17000000.gv11b`, `gv11b` and `gv11b.0` all resolve.
 * Returns undefined for anything that is not a generation-bearing node.
 */
function matchGpuArchitecture(entry) {
  const match = TEGRA_GPU_ARCH_NODE.exec(entry);
  if (!match) {
    return undefined;
  }
  const node = match[1].toLowerCase();
  return { node, ...TEGRA_GPU_ARCHITECTURES[node] };
}

/**
 * Scan the places a Tegra GPU node appears until one names a generation.
 *
 * Undefined means "this board did not tell us", which is an ordinary answer on
 * a release that only exposes the generic alias - never an error.
 */
async function readGpuArchitecture(fsImpl, rootDir) {
  for (const dir of GPU_ARCH_DIRS) {
    for (const entry of await listDir(fsImpl, path.join(rootDir, dir))) {
      const match = matchGpuArchitecture(entry);
      if (match) {
        return match;
      }
    }
...
async function readJetsonTelemetry({ rootDir = '/', fsImpl = fsPromises } = {}) {
  ...
  const [model, powerStatus, powerConfig, loadPermille, clocks, temperatureC, gpu] = await Promise.all([
    ...
    readGpuArchitecture(fsImpl, rootDir)
  ]);
