 ...
 */
// The prompt has to share num_ctx with the answer. Leave room for a Signal K
// context of roughly this size before the model's own reply starts.
...
function buildRuntimeOptions(config) {
  ...
  const answerBudget = Math.max(64, config.numCtx - PROMPT_HEADROOM_TOKENS);
