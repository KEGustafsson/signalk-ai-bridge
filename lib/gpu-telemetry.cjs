...
function formatBytes(bytes) {
  // `< 1`, not `<= 0`: for a fractional byte count Math.log is negative, so the
  // exponent went to -1 and units[-1] rendered as "512.0 undefined" in the
  // operator-facing residency message.
